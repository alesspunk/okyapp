...
export function mountOkyCashPrototype(root, { userType = "first-time" } = {}) {
  ...
  function startFinish() {
    ...
    const relay = (n) => {
      ...
      tourFlagTimer = setTimeout(() => {
        ...
        /* Con el confeti arranca el reloj: el strip pasa de aqua al 5%
           a mostaza con cuenta atrás, y ese cambio a la vista explica
           solo lo que una parada del recorrido tendría que contar. */
        startPromo();
        render();
        tourFlagTimer = setTimeout(() => {
          state.tourConfetti = false;
  ...
  /* Quien entra por el enlace directo de USA no pasa por la
     ...
     al montar. Entrando por el folder manda la presentación, que lo
     arranca al final. */
  if (state.screen === "home") startPromo();
