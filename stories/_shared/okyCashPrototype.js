...
/* Daniel Paz lleva sus iniciales aquí y no solo en la agenda: en
   Guatemala es el destinatario de salida, y sin ellas el pago abría con
   el muñeco genérico hasta que se pasara por la agenda. */
const GUA_RECIPIENT = { name: "Daniel Paz", phone: "+502 6578-8744", initials: "DP" };
...
function createInitialState(userType) {
  ...
  return {
    ...
    helpOn: false,
    /* La cuenta se cumplió fuera de la home: el reloj arranca al
       volver, no a espaldas de nadie. */
    promoPending: false,
    tourCount: false,
    tourReady: false,
    tourFlag: false,
...
function homeHeader(state, headerState) {
  return renderDiscoveryHeader({
    ...
    showPlateu: false,
    /* Bajando se queda arriba el header con el wallet y el carrito, y
       debajo el folder minimizado y el buscador. Antes el header era
       justo lo que se iba, y con él se perdían de vista dónde están
       los vales comprados —con su punto rojo— y qué hay en el
       carrito. */
    keepAppHeader: true,
...
export function mountOkyCashPrototype(root, { userType = "first-time" } = {}) {
  ...
  /* Si Clarita ya estaba en el dibujo anterior no vuelve a presentarse. */
  let claritaWasThere = false;
  let addedTimer = null;
  ...
  /* Repintar reconstruye el contenedor que scrollea, así que la página
     se va al tope. Navegando es lo correcto —una pantalla nueva empieza
     arriba—, pero cuando lo que cambia le pasa a la página que ya se
     está leyendo (el reloj que arranca, el que vence) mandarla al
     principio es arrancarle la lectura de las manos. Esos repintados
     piden keepScroll y se quedan donde estaban. */
  function render({ keepScroll = false } = {}) {
    ...
    /* La promo que se quedó esperando fuera de la home arranca en
       cuanto se vuelve, con el mismo respiro que tras la presentación:
       el confeti y el cruce de color son para verlos. */
    if (state.promoPending && state.screen === "home") {
    ...
    /* Clarita saluda al llegar, no en cada repintado. Cualquier toque
       —cambiar una cantidad, abrir una sección— vuelve a dibujar la
       pantalla, y con ella volvían a correr la entrada, el saludo y la
       frase: se la veía aparecer una y otra vez. Si ya estaba en el
       dibujo anterior, se queda quieta y solo respira. */
    const clarita = root.querySelector(".prime-card-clarita, .oky-flow-guide");
    if (clarita && claritaWasThere) clarita.classList.add("is-settled");
    claritaWasThere = !!clarita;
    ...
    if (state.cartOpen) {
      ...
      /* Cambiar la cantidad repinta el carrito, y con él volvían a
         correr la entrada desde el lado y el fundido del fondo: parecía
         que se cerraba y se abría en cada toque. Las dos animaciones son
         para abrirlo, no para cada cambio de dentro. */
      if (cartWasOpen) {
        frame
          .querySelectorAll(".oky-flow-drawer, .oky-flow-drawer-backdrop")
          .forEach((el) => {
  ...
  function startPromo() {
    ...
    /* Y solo arranca en la home. El confeti y el cruce de aqua a
       ...
       PDP o el wallet caía confeti sobre otra cosa y el cintillo
       cambiaba a espaldas de la persona, que volvía a la home con el
       20% ya puesto sin haber visto el salto. Fuera de la home la
       cuenta se queda esperando y arranca al volver. */
  ...
  function bindClaritaInView(scroll) {
    ...
    /* Mover el nodo de padre reinicia sus animaciones, así que se hace
       solo cuando de verdad cambia de sitio: colgándola y descolgándola
       en cada medida —y se mide en cada scroll— volvía a entrar una y
       otra vez, que es el parpadeo que se veía. */
    const hang = (pinned, bottom) => {
      if (pinned) {
        if (clarita.parentElement !== frame) {
          frame.appendChild(clarita);
          clarita.classList.add("is-pinned");
        }
        if (clarita.style.bottom !== bottom) clarita.style.bottom = bottom;
        return;
      }
      if (clarita.parentElement !== home) {
        home.appendChild(clarita);
    ...
    const place = () => {
      ...
      /* En un teléfono el lienzo va escalado, así que lo medido y lo
         que se escribe en píxeles no son la misma unidad. */
      const zoom = box.height / frame.offsetHeight || 1;
      const tall = clarita.offsetHeight * zoom;
      ...
      /* Dónde caería si estuviera en su sitio. Se pregunta al hueco de
         la card y no a ella: el hueco sigue ahí aunque esté colgada, y
         así no hace falta devolverla para medir —devolverla y volver a
         colgarla en cada scroll es lo que la hacía parpadear—. */
      const wouldBottom = home.getBoundingClientRect().top + tall;
      ...
      if (wouldBottom <= limit) hang(false);
      else hang(true, `${Math.round((box.bottom - limit) / zoom)}px`);
    };
  ...
  root.addEventListener("click", (event) => {
    ...
    if (action === "menu:wallet") {
      /* Manda lo que esté sin abrir: gift cards primero, después vales
         y servicios. Sin novedades abre en gift cards, que es lo que
         más se guarda. A diferencia del icono del header, aquí no
         decide el país: el menú no es de ninguna tienda. */
      state.walletFilter = "";
      state.openBeforeFilter = null;
    ...
    if (action === "nav:wallet") {
      /* De dónde se entra decide dónde te deja. Desde una tienda, el
         ...
         lo que hay ahora. */
      /* Y si hay algo sin abrir, manda eso: el punto rojo del icono
         promete llevarte a lo nuevo, así que abre la pestaña y la
         sección donde de verdad está. Antes te dejaba en la de siempre
         y había que ir a buscarlo. */
      const fromPurchase = POST_PURCHASE.includes(state.screen) || hasNewVouchers(state);
      state.walletFilter = "";
      state.openBeforeFilter = null;
      /* La pestaña de la tienda de la que se entra, en la versión que
         esté abierta: en la de siempre es el tipo que esa tienda vende
    ...
    if (action === "go:decision") {
      /* En Guatemala el destinatario lo pone la propia orden, así que
         no hay nada que preguntar: se va derecho al pago y state.recipient
         sigue siendo lo que la persona eligió para sus compras de USA. */
